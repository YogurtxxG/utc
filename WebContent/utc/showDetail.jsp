	...
	$(function() {
		$('#queryTable').datagrid({
			...
			toolbar : [{
				...
				iconCls : 'icon-back',
				handler : function() {
							window.location.href='<%=request.getContextPath()%>/utc/check/show.jsp';
						}
			}]
		});
	});
...
<body>
	<div id="panel" class="easyui-panel" title="报销申请单"
		...
		<form id="addForm" method="post" action="<%=request.getContextPath()%>/utc/reimbursement/AddToReimbursement.action">
			...
			<tr>	
			<td>编号:</td>
			<td><input id="number" type="text" name="number" value="<%=numId %>" size="38" disabled="disabled"/></td>
			<td></td>
			<td>项目:</td>
			<td><select id="projectName" class="easyui-combobox"
						name="projectName" url="<%=request.getContextPath()%>/app/system/dict/listDictByType.action?type=utc.project"
						 valueField="id" textField="text" editable="false" disabled="disabled" ></select></td>
			<td></td>
			<td>时间:</td>
			<td><input id="date" type="text" name="date"  class="easyui-datebox" value="<%=date %>" disabled="disabled"></input></td>
			<td></td>
			<td>员工名:</td>
			<td><input id="userName" type="text" name="userName" value="<%=name %>" readonly="readonly" disabled="disabled"></input><td>
			</tr>
			<tr>
				<td>备注:</td>
